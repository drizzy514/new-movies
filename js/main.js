var filmsWrapper = document.querySelector(".col-md-12");
...
var getGaner = function () {
    var ganres = [];
    films.forEach(function (film) {
        film.genres.forEach(function (genre) {
            if (!ganres.includes(genre)) {
                ganres.push(genre)
...
var filmGanres = getGaner();
var formSelect = document.querySelector("#ganer");

filmGanres.forEach(function (genre) {
    var genreOption = document.createElement("option");
    genreOption.value = genre;
    genreOption.textContent = genre;

    formSelect.append(genreOption)
})
...
var filmTitles = getTitle();
filmTitles.forEach(function (title) {


})

var wrapper = function (filmWrapper) {
    filmsWrapper.innerHTML = "";
    filmWrapper.forEach(function (film) {

        var createdFilm = document.createElement("article");
        var filmPoster = document.createElement("img");
        filmPoster.src = film.poster;
        createdFilm.append(filmPoster);

        var filmTitle = document.createElement("h2");
        filmTitle.textContent = film.title;
        createdFilm.append(filmTitle);

        var filmId = document.createElement("p");
        filmId.textContent = film.id;
        createdFilm.append(filmId);
        filmId.classList.add("film-id");

        var filmsOverview = document.createElement("p");
        filmsOverview.textContent = film.overview;
        createdFilm.append(filmsOverview);


        var filmsReleaseDate = document.createElement("p");
        filmsReleaseDate.textContent = film.release_date;
        createdFilm.append(filmsReleaseDate)
        filmsReleaseDate.classList.add("release-date");

        var filmLinks = document.createElement("div")
        for (var genre of film.genres) {
            var filmsLink = document.createElement("a");
            filmsLink.textContent = genre;
            filmLinks.append(filmsLink);
            filmsLink.href = "#";

        }
        createdFilm.append(filmLinks);
        filmsWrapper.append(createdFilm);

        filmsBtn = document.createElement("button");
        createdFilm.append(filmsBtn);
        filmsBtn.textContent = "See Movie";
        filmsBtn.classList.add("mb-3")


        filmsButton = document.createElement("button");
        createdFilm.append(filmsButton);
        filmsButton.textContent = "Watch trailer";
        filmsButton.classList.add("btn-outline-primary");
...
form.addEventListener("submit", function (evt) {
    evt.preventDefault();
    var inputValue = new RegExp(formInput.value, "gi");
    var genreValue = document.querySelector("select").value;
    var filtered = films.filter(function (film) {
    	if (genreValue === "0") {
    		return true;
    	}

    	return film.genres.includes(genreValue)
    }).filter (function (film){
        return film.title.match(inputValue)
    })
